...
| **S6** ✅ DONE 2026-07-03 | Verifiable ledger v1: `pkg/ledger` — deterministic FSM hash chain (`chain[i]=SHA256(prev‖index‖term‖cmd)`, persisted head in `__ledger`), **BLS12-381 (G2 keys ⇒ 48-byte G1 signatures, aggregatable)** node keys + **Ed25519 CA** certifying them (with proof-of-possession), Checkpoint + QuorumCertificate (48-byte aggregate + signer set, offline-verifiable) — the smallest multi-signer footprint. `ledger.digest` API (cdb.proofs.read; auditor role). CLI `ledger ca-init\|keygen\|issue\|verify`. Proven by `TestLedgerQuorumOverConvergedChain` (3 replicas converge → 2-of-3 quorum verifies offline, tamper fails) + `pkg/ledger` unit tests + CLI interop. Live leader-driven checkpoint gossip + WORM anchoring = follow-up | M |
| **S7** ✅ DONE 2026-07-03 | Admin console: HTTP/JSON REST API (`pkg/console`, bearer/basic auth → principal → authorize) since the value-rpc plane has no JS client; **background verify-jobs** (`POST /api/ledger/verify` → id, `GET …/verify/{id}` → {state,progress,result}); **`pkg/verify` backup-verifier** (loads a dump, reads the chain head, matches a quorum certificate — offline, progress-reporting) reused by the **`ledger verify-backup` CLI** and the REST job; **Vue+Vite SPA in `webapp/`** (cluster + ledger-head + backup-verify progress bar) served at `/console`, built into the Docker image. Ledger verify API + offline backup verification. Proven by `TestVerifyBackupAgainstQuorum` + `pkg/console` REST/job tests | M–L |
